		...
		<form enctype="multipart/form-data" method="post" class="text-center">
			 <div class="col-md-12">
					...
					<label>
						...
						Blood Group:<select type="text" name="bgroup" value="" placeholder="address"> <?php
						include('config.php');
						$result = $conn->query("select bg_name from bloodgroup");
        echo '<option value=""  >select</option>';
    while ($row = $result->fetch_assoc()) {

								?>	
										
<option><?php echo htmlentities($row['bg_name']);?>
																</option>
																<?php } ?>										
